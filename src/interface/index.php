	...
	case "add":
		if(!empty($_POST["quantity"])) {
			...
			$result=mysqli_query($con,"SELECT * FROM products WHERE id='$pid'");
	          while($pname=mysqli_fetch_array($result)){
			$itemArray = array($pname["ProductName"]=>array('ProductName'=>$pname["ProductName"], 'Price'=>$_POST["Price"], 'image'=>$_POST["image"], 'Description'=>$POST["Description"], '1'));
			if(!empty($_SESSION["cart_item"])) {
				if(in_array($Pname["ProductName"],array_keys($_SESSION["cart_item"]))) {
					foreach($_SESSION["cart_item"] as $k => $v) {
							if($pname["ProductName"] == $k) {
